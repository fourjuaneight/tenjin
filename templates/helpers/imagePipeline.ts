...
/**
 * Create various size and format variants of an image based on the asset type.
 */
(() => {
   ...
   *
   * @param data Variants options
   * @param src  Image to format and convert
   * @param dest Where to save image
   */
  const fmtImage = async (
    ...
    const promises = data.map(async img => {
      ...
      await image
        ...
        .toFile(output)
        .then(() => {
          console.info('[SUCCESS]', `${fileName} created.`);
        })
        .catch(err => {
          console.error('[ERROR]', {
            input,
  ...
  // run script based on asset type
  switch (argv.asset) {
    case 'favicon':
    ...
    case 'about':
      ...
      break;
    default:
      console.info('[INFO]', 'No images converted.');
      break;
  }
})();
